...
def parse_status(html, users):
    ...
    if node_idx == -1:
        return dict(studentActive=0, researcherActive=0, freeMIG=0,
                    studentLimit=STUDENT_LIMIT, researcherLimit=RESEARCHER_LIMIT,
                    timestamp=None, error='Node not found in status page')

    node_block = text[node_idx:node_idx + 4000]

    # Find all jupyter-USERNAME pods on this node
    ...
    # Count free MIG slices on this node
    free_mig = 0
    for m in re.finditer(r'nvidia\.com/mig-[\w.]+\s+(.*?)(?=nvidia\.com/|$)', text):
